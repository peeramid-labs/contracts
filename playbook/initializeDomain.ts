...
task('initializeDomain', 'Just a test task')
  ...
  .setAction(async ( { domain, fee, reward, discount, registrarAddress }: { domain: string; fee: string; reward: string; discount: string, registrarAddress: string }, hre) => {
    const { deployments, getNamedAccounts } = hre
    const { owner, registrar } = await getNamedAccounts();
    const multipassDeployment = await deployments.get('Multipass');
    hre.multipass = new hre.ethers.Contract(
      multipassDeployment.address,
      multipassDeployment.abi,
      hre.ethers.provider.getSigner(owner),
    ) as MultipassDiamond;
    const tx = await hre.multipass
      .initializeDomain(
        registrarAddress ?? hre.ethers.provider.getSigner(registrar).address,
        1000,
