...
library LibCoinVending {
  ...
  bytes32 constant COIN_VENDING_STORAGE_POSITION =
    keccak256("coin.vending.storage.position");

  function coinVendingPosition(
    bytes32 position
  ) internal view returns (Condition storage) {
    return coinVendingStorage().positions[keccak256(abi.encode(position))];
  }

  function coinVendingStorage()
    internal
    pure
    returns (LibCoinVendingStorage storage es)
  {
  ...
  function fulfill(
    ...
    for (uint256 i = 0; i < position.contractAddresses.length; i++) {
      ...
      ContractCondition storage requirement = position.contracts[contractType][
        contractAddress
      ][id];
  ...
  function _refund(Condition storage reqPos, address to) private {
    require(
      (reqPos.timesRefunded + reqPos.timesReleased) < reqPos.timesFunded,
  ...
  function configure(
    bytes32 position,
    ConfigPosition memory configuration
  ) internal {
    Condition storage mustDo = coinVendingPosition(position);
    require(
      mustDo.timesFunded == 0 ||
        (mustDo.timesFunded == (mustDo.timesRefunded + mustDo.timesReleased)),
    ...
    for (uint256 i = 0; i < configuration.contracts.length; i++) {
      ...
      mustDo.contracts[configuration.contracts[i].contractType][
        configuration.contracts[i].contractAddress
      ][configuration.contracts[i].contractId] = configuration
        .contracts[i]
        .contractRequirement;
    }
  ...
  function getPosition(
    bytes32 position
  ) internal view returns (ConditionReturn memory) {
